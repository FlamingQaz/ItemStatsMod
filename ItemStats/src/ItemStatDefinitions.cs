...
namespace ItemStats
{
	internal partial class ItemStatProvider
	{
		...
		public static void Init()
		{
			...
			// Hunter's Harpoon
			ItemDefs[ItemCatalog.FindItemIndex("MoveSpeedOnKill")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
						(itemCount, ctx) => 1f + (itemCount - 1f) * 0.5f,
						(value, ctx) => $"Total Duration: {value.FormatInt("s", 1)}"
					),
				}
			};
			// Symbiotic Scorpion
			ItemDefs[ItemCatalog.FindItemIndex("PermanentDebuffOnHit")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
			...
			// Mocha
			ItemDefs[ItemCatalog.FindItemIndex("AttackSpeedAndMoveSpeed")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
						(itemCount, ctx) => itemCount * 0.075f,
						(value, ctx) => $"Attack Speed Increase: {value.FormatPercentage()}"
					),
					new ItemStat(
						(itemCount, ctx) => itemCount * 0.07f,
						(value, ctx) => $"Move Speed Increase {value.FormatPercentage()}"
					),
				}
			};
			// Laser Scope
			ItemDefs[ItemCatalog.FindItemIndex("CritDamage")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
						(itemCount, ctx) => itemCount,
						(value, ctx) => $"Additional Damage {value.FormatPercentage()}"
					),
				}
			};
			// Safer Spaces
			ItemDefs[ItemCatalog.FindItemIndex("BearVoid")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
						(itemCount, ctx) => Mathf.CeilToInt(15f * Mathf.Pow(0.9f, itemCount)),
						(value, ctx) => $"Recharge Duration {value.FormatInt("s", 1)}"
					),
				},
			};
			// Weeping Fungus
			ItemDefs[ItemCatalog.FindItemIndex("MushroomVoid")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
						(itemCount, ctx) => 0.01f * itemCount,
						(value, ctx) => $"Heal Amount: {value.FormatPercentage()}"
					),
				},
				AdditionalText = "Description is incorrect".SetColor("red")
			};
			// Benthic Bloom
			ItemDefs[ItemCatalog.FindItemIndex("CloverVoid")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
						(itemCount, ctx) => 3f * itemCount,
						(value, ctx) => $"Items Upgraded: {value.FormatInt()}"
					),
				},
			};
			// Ignition Tank
			ItemDefs[ItemCatalog.FindItemIndex("StrengthenBurn")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
			...
			// Needletick
			ItemDefs[ItemCatalog.FindItemIndex("BleedOnHitVoid")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
						(itemCount, ctx) => 0.1f * itemCount,
						(value, ctx) => $"Collapse Chance: {value.FormatPercentage()}"
					),
				},
			};
			// Lost Seer's Lenses
			ItemDefs[ItemCatalog.FindItemIndex("CritGlassesVoid")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
			...
			// Tentabauble
			ItemDefs[ItemCatalog.FindItemIndex("SlowOnHitVoid")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
						(itemCount, ctx) => itemCount * 0.05f,
						(value, ctx) => $"Root Chance: {value.FormatPercentage()}"
					),
					new ItemStat(
						(itemCount, ctx) => itemCount,
						(value, ctx) => $"Root Duration: {value.FormatInt("s")}"
			...
			// Plasma Shrimp
			ItemDefs[ItemCatalog.FindItemIndex("Plasma Shrimp")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
						(itemCount, ctx) => itemCount * 0.4f,
						(value, ctx) => $"Total Damage: {value.FormatPercentage()}"
					),
					new ItemStat(
						(itemCount, ctx) => 1,
						(value, ctx) => $"Missile Count: {value.FormatInt()}"
					),
				},
			};
			// Polylute
			ItemDefs[ItemCatalog.FindItemIndex("ChainLightningVoid")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
						(itemCount, ctx) => itemCount * 3f,
						(value, ctx) => $"Total Strikes: {value.FormatInt()}"
					),
				},
			};
			// Lysate Cell
			ItemDefs[ItemCatalog.FindItemIndex("EquipmentMagazineVoid")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
			...
			// Voidsent Flame
			ItemDefs[ItemCatalog.FindItemIndex("ExplodeOnDeathVoid")] = new ItemStatDef
			{
				Stats = new List<ItemStat>
				{
					new ItemStat(
						(itemCount, ctx) => 3.5f * (1f + (itemCount - 1) * 0.8f),
						(value, ctx) => $"Base Damage: {value.FormatPercentage()}"
					),
					new ItemStat(
						(itemCount, ctx) => 12f + 2.4f * (itemCount - 1f),
						(value, ctx) => $"Radius: {value.FormatInt("m")}"
